...
SELECT p.id, le.registration_area,
 ...
 WHERE cc.kwd_system = 'eHealth/ICPC2/condition_codes' AND cc.code IN ('K86', 'K87') AND cc.is_current = 'Y' AND c.is_current = 'Y' AND (p.status = 'active' OR EXTRACT(YEAR FROM p.death_date) > 2022) AND EXTRACT(YEAR FROM c.onset_date) < 2024)   
