...
SELECT p.id, le.registration_area,
 ...
 WHERE cc.kwd_system = 'eHealth/ICPC2/condition_codes' AND c.verification_status = 'confirmed' 
   AND cc.code IN ('K86', 'K87') AND cc.is_current = 'Y' AND c.is_current = 'Y' 
   AND p.is_current = 'Y' AND (p.status = 'active' OR EXTRACT(YEAR FROM p.death_date) > 2022) 
